...
if (import.meta.env.DEV) {
  ...
  (window as any).__LOG = {
    setFilter: (f: string) => { localStorage.setItem('LOG', f); console.log(`[LOG] filter set to: "${f}" — reload not required`); },
    getFilter,
    silence: () => { localStorage.setItem('LOG', ''); console.log('[LOG] silenced'); },
    all:     () => { localStorage.setItem('LOG', '*'); console.log('[LOG] all categories enabled'); },
  };
}
