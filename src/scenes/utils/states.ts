...
import { Duration, Ease, Distance, tween, type TweenTarget } from './motion';

export type StateName = keyof typeof State;

/**
 * Duck-typed game object surface accepted by applyState.
 *
 * Using a plain interface (not extending Phaser's abstract base) avoids
 * conflicts with Phaser's strictness on required abstract members. Callers
 * pass Phaser GameObjects which are structurally compatible at runtime.
 */
export interface StylableGameObject {
  setScale?(scale: number): this;
  setAlpha?(alpha: number): this;
  setInteractive?(config?: boolean | object): this;
  ...
  width?: number;
  height?: number;
  // Mutable runtime slots for managed sub-objects
  _focusRing?: Phaser.GameObjects.Graphics | undefined;
  _spinner?: Phaser.GameObjects.Container | undefined;
}
...
export function applyState(
  ...
  if (def.shake) {
    ...
    const tweenTarget = target as unknown as TweenTarget;
  ...
  if (def.spinner) {
    ...
    tween(
      scene,
      spinner as unknown as TweenTarget,
      { rotation: Math.PI * 2 },
      {
        duration: 1000,
        ease: Ease.out,
  ...
  if (def.motion) {
    ...
    if (Object.keys(tweenProps).length > 0) {
      tween(scene, target as unknown as TweenTarget, tweenProps, {
        duration: durationMs,
        ease,
      });
    }
