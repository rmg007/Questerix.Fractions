...
describe('validator.label.matchTarget', () => {
  ...
  it('returns correct when all mappings are correct', () => {
    const result = fn(
      {
        studentMappings: [
          { labelId: 'label-half', regionId: 'r1' },
          { labelId: 'label-half', regionId: 'r2' },
        ],
      },
      expected
    );
  ...
  it('returns incorrect for wrong label on region', () => {
    const result = fn(
      {
        studentMappings: [
          { labelId: 'label-third', regionId: 'r1' },
          { labelId: 'label-half', regionId: 'r2' },
        ],
      },
      expected
  ...
  it('returns incorrect for unmapped region', () => {
    const result = fn({ studentMappings: [{ labelId: 'label-half', regionId: 'r1' }] }, expected);
    expect(result.outcome).toBe('incorrect');
  });
    ...
    const result = fn(
      {
        studentMappings: [
          { labelId: 'label-half', regionId: 'r1' },
          { labelId: 'label-third', regionId: 'r2' },
        ],
      },
      expected
    );
