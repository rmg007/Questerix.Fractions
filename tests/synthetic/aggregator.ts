...
export function aggregate(sessions: SessionRecord[]): AggregatedReport {
  ...
  const meanDuration =
    totalDurations.reduce((a, b) => a + b, 0) / Math.max(totalDurations.length, 1);
  ...
  const feedbackLatencyPass = latencies.length > 0 ? fastFeedback / latencies.length >= 0.9 : true; // no data = don't fail
  ...
  for (const name of personaNames) {
    ...
    perPersona[name] = {
      ...
      meanResponseTimeMs:
        pAttempts.reduce((a, b) => a + b.responseTimeMs, 0) / Math.max(pAttempts.length, 1),
    };
  }
  ...
  if (!feedbackLatencyPass) {
    failReasons.push(
      `Feedback latency p95=${feedbackLatencyP95Ms}ms — <90% of attempts within 800ms budget`
    );
  }
...
export function printSummary(report: AggregatedReport): void {
  ...
  console.log(
    `  Sessions:        ${report.totalSessions} total | ${report.completedSessions} completed | ${report.abandonedSessions} abandoned | ${report.crashedSessions} crashed`
  );
  ...
  console.log(
    `  Mean duration:   ${ms(report.meanSessionDurationMs)} | p95: ${ms(report.p95SessionDurationMs)}`
  );
  console.log(
    `  Feedback <800ms: ${report.feedbackLatencyPass ? 'PASS' : 'FAIL'} (p95=${ms(report.feedbackLatencyP95Ms)})`
  );
  console.log('\n  Per-persona breakdown:');
  for (const [name, p] of Object.entries(report.perPersona)) {
    console.log(
      `    ${name.padEnd(18)} sessions=${p.sessions} complete=${p.completed} abandon=${p.abandoned} crash=${p.crashed} accuracy=${pct(p.meanAccuracy)} avgResp=${ms(p.meanResponseTimeMs)}`
    );
  }
