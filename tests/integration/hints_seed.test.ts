...
// Unconditional: hints.json is auto-generated by build:curriculum in all CI/test workflows,
// so it's guaranteed to exist when npm run test:integration runs.
describe('Hints Seeding Integration', () => {
  ...
  it('should seed all hints to hintRepo without errors', async () => {
    ...
    if (hintsData.length > 0) {
      const firstTemplate = hintsData[0]!.questionTemplateId as any;
  ...
  it('should retrieve hints by individual id', async () => {
    ...
    const retrieved = await hintRepo.get(testHint.id as any);
  ...
  it('should query hints by questionTemplateId with correct ordering', async () => {
    ...
    if (testTemplate) {
      const hints = await hintRepo.getForQuestion(testTemplate as any);
  ...
  it('should handle empty result for non-existent template', async () => {
    ...
    const hints = await hintRepo.getForQuestion('q:xx:L0:0000' as any);
