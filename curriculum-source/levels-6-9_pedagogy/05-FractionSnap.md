...
function calculateRating(score: GameScore): GameScore['finalRating'] {
  ...
  if (score.accuracy >= 0.70) return 'Silver';
  return 'Bronze';
}
```
...
const pairBank: FractionPair[] = [
  ...
  { a: {num:3,den:4}, b: {num:1,den:4}, difficulty: 0.10 },
  ...
  { a: {num:4,den:7}, b: {num:3,den:5}, difficulty: 0.50 },
...
function selectPair(tier: Difficulty, recentPairs: FractionPair[]): FractionPair {
  const pool = pairBank.filter(p => 
    p.difficulty >= tierRange[tier].min &&
    p.difficulty <= tierRange[tier].max
  );
