...
- Watch for: kid solves Q1/Q2 quickly, then suddenly sees a 4-equal-parts prompt with one drag-line — `Level01Scene.ts:99-104` template `q:pt:L1:0003` is medium-tier "Split this shape into 4 equal parts" and BKT auto-promotes after correct answers (`selectNextQuestion` at `Level01Scene.ts:650-690`). One handle cannot satisfy a 4-part task.
...
- Watch for: kid drags a label tile, drops it anywhere, gets credit/no-credit unpredictably — `LabelInteraction.ts:90-94` snapToFirst is the keyboard/test path that always pins to region 0; the canvas-drag path (`:102-111`) snaps to the *nearest* region. If a kid uses the keyboard a11y action they lock to region 0 regardless of intent.
