...
|---|---|---|---|
| T1 | Redesign: bottom-sheet, solid colors, 1400/1600 ms timing, real shake | `FeedbackOverlay.ts`, `SFXService.ts` | [x] |
| T5 | Wrong-answer sound | `SFXService.ts` | [x] |

### Group B — Quest expressions

| Task | Description | Files | Done |
|---|---|---|---|
| T2 | `oops` expression: droop + wince eyes + sweat drop on wrong | `Mascot.ts`, `Level01Scene.ts`, `LevelScene.ts` | [x] |
| T9-fix3 | `oops` expression in onboarding wrong-answer moment | `OnboardingScene.ts` | [x] |

### Group C — Partition interaction

| Task | Description | Files | Done |
|---|---|---|---|
| T3 | Solid navy partition line, sky-tint shape background | `Level01Scene.ts`, `PartitionInteraction.ts` | [x] |
| T8 | Ghost midpoint after 1st wrong answer | `Level01Scene.ts`, `PartitionInteraction.ts` | [x] |
| T13 | Snap juice: tint fills + fraction labels + snap sound | `PartitionInteraction.ts`, `SFXService.ts` | [x] |

### Group D — Small isolates

| Task | Description | Files | Done |
|---|---|---|---|
| T4 | Decouple TTS from Reduce Motion → `ttsEnabled` pref | `deviceMeta.ts`, `SettingsScene.ts` | [x] |
| T6 | "Back to Menu" 2-tap protection | `Level01Scene.ts`, `LevelScene.ts` | [x] |
| T7 | Auto-hint on 3rd wrong answer | `Level01Scene.ts`, `LevelScene.ts` | [x] |
| T10 | Prompt 22→28 px, counter 17→22 px | `Level01Scene.ts`, `LevelScene.ts` | [x] |

### Group E — Session complete

| Task | Description | Files | Done |
|---|---|---|---|
| T11 | Advance/stay/regress call-to-action banner | `SessionCompleteOverlay.ts`, `Level01Scene.ts`, `LevelScene.ts` | [x] |
| T15 | Perfect 5/5 gold overlay + doubled confetti | `SessionCompleteOverlay.ts`, `SFXService.ts` | [x] |

### Group F — Quest personality

| Task | Description | Files | Done |
|---|---|---|---|
| T14 | Idle/boredom 3-stage escalation at 10/18/28 s | `Mascot.ts`, `Level01Scene.ts`, `LevelScene.ts` | [x] |
| T16 | Speech bubbles at 10 key moments | `Mascot.ts`, `Level01Scene.ts`, `LevelScene.ts` | [x] |

### Group G — Streaks

| Task | Description | Files | Done |
|---|---|---|---|
| T12 | "3 in a Row! 🔥" mid-session banner | `Level01Scene.ts`, `LevelScene.ts` | [x] |
| T17 | Daily streak pill in Menu + LevelMap | `MenuScene.ts`, `LevelMapScene.ts` | [x] |

### Group H — Onboarding

| Task | Description | Files | Done |
|---|---|---|---|
| T9-fix1 | Hand pointer at SHAPE_CY, not below shape | `OnboardingScene.ts` | [x] |
| T9-fix2 | Tap-to-advance Step 1 | `OnboardingScene.ts` | [x] |
