...
- [x] S1-T1 — Wire `updateMastery()` in `Level01Scene.recordAttempt()` (G-E1)
- [x] S1-T2 — Wire `runAllDetectors()` in `Level01Scene.recordAttempt()` (G-E2; already wired in `LevelScene` per review)
- [x] S1-T3 — Pass `hintsUsedIds` through to attempt records (G-E3)
- [x] S1-T4 — Real `accuracy` + `avgResponseMs` in `closeSession()` (G-E4)
- [x] S1-T5 — Verify state transitions in IndexedDB: `NOT_STARTED → LEARNING → APPROACHING → MASTERED` (`tests/integration/mastery-transition.test.ts`)
- [x] S1-T6 — `[archetype+submittedAt]` index landed in schema v4 (current schema v7); G-DB1 closed
...
- [x] S2-T1 — Unlock model resolved: **session-completion gate** (`levelUnlockRepo` unlocks N+1 on N completion)
- [x] S2-T2 — Adventure-map nodes tappable (`MenuScene._renderLevelGrid()` 404-507)
- [x] S2-T3 — "Keep going" advances `levelNumber` (`LevelScene` 1030-1050; `Level01Scene` 1323-1336 → L2)
- [x] S2-T4 — L2 authored: 40 templates (partition + identify, includes thirds + quarters)
...
- [x] S3-T1 — `tts.speak(promptText)` called on question load (`LevelScene.ts:272`); 5 call-sites for warmup + announcements
- [ ] S3-T2 — iPad Safari TTS + touch-drag test (G-OPS2) — *manual, requires device*
- [ ] S3-T3 — Playtest feedback animations with a 6-year-old (or close proxy) (G-UX4) — *manual*
...
- [x] S4-T1 — L3-L5 authored: L3=25 (equal_or_not + label), L4=14 (make), L5=12 (snap_match)
- [x] S4-T2 — L6-L9 authored: L6=42 (compare), L7=42 (compare), L8=42 (benchmark), L9=42 (order)
- [ ] S4-T3 — Per-level browser smoke test
- [ ] S4-T4 — Mastery-gated unlock — *deferred; D-1 resolved as completion-gate; revisit if needed*
